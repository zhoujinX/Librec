 ...
 * the {@code UserCluster} recommender.//这是一个图模型用于将物品分别聚集在K个组用与UserCluster对比
 *
...
public class ItemClusterRecommender extends ProbabilisticGraphicalRecommender {
    ...
    protected void setup() throws LibrecException {
        ...
        Collections.sort(ratingScale);//对分数List排序
        numTopics = conf.getInt("rec.pgm.number", 10);//取出"rec.pgm.number"代表数字
        												//这里"rec.pgm.number"=10
        topicRatingProbs = new DenseMatrix(numTopics, numRatingLevels);//创建稠密矩阵，行代表排名，列代表每个物品
        for (int k = 0; k < numTopics; k++) {//取前10的评分
            double[] probs = Randoms.randProbs(numRatingLevels);//numRatingLevels表示评分级别
            for (int r = 0; r < numRatingLevels; r++) {
                topicRatingProbs.set(k, r, probs[r]);//计算一个初始概率
            }
        }

        topicInitialProbs = new DenseVector(Randoms.randProbs(numTopics));

        itemTopicProbs = new DenseMatrix(numItems, numTopics);//每个物品的前10评分

        itemNumEachRating = new DenseMatrix(numItems, numRatingLevels);//横坐标是每件物品，纵坐标是评分等级
        ...
        for (int i = 0; i < numItems; i++) {
            ...
            for (VectorEntry vi : ri) {
                ...
                itemNumEachRating.add(i, r, 1);//记录每一个项目的评分
            }
            itemNumRatings.set(i, ri.size());//记录单个项目的评分
        }
    ...
    protected double predict(int userIdx, int itemIdx) throws LibrecException {
        ...
        for (int k = 0; k < numTopics; k++) {
            ...
            for (int r = 0; r < numRatingLevels; r++) {
                ...
                pred_k += rij * pkr;//评分乘上初始概率
            }

            pred += pi_k * pred_k;//针对user，用pred_k乘上pi_k预测user评分
        }
