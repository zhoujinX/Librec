...
public abstract class AbstractRecommender implements Recommender {
     ...
     */
    public RecommendedList recommendedList;
    ...
    public void setRecommendedList(RecommendedList r) throws IllegalAccessException, InstantiationException, InvocationTargetException, NoSuchMethodException {
    	this.recommendedList=(RecommendedList) BeanUtils.cloneBean(r);
    }
    ...
    /*将推荐列表返回*/
    public RecommendedList recommended(RecommenderContext context) throws LibrecException{
    	this.context = context;
        setup();
        LOG.info("Job Setup completed.");
        trainModel();
    ...
    /**
     * recommend
     * * predict the ranking scores or ratings in the test data预测排名或评分
     *
...
//            recommendedList.topNRankItemsByUser(userIdx, topN);//省略后，两个算法的推荐列表一致,没有topN
        }
    ...
    public List<RecommendedItem> getRecommendedList() {
        if (recommendedList != null && recommendedList.size() > 0) {
            ...
            if (userMappingData != null && userMappingData.size() > 0 && itemMappingData != null && itemMappingData.size() > 0) {
                BiMap<Integer, String> userMappingInverse = userMappingData.inverse();//key、value转换
